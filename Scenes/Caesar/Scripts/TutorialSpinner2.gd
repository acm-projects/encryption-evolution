...
func _on_Backspace_pressed():
	if num_letters_pressed > 22: # TODO Use variable size instead of hard coding
		num_letters_pressed = 22
	num_letters_pressed = num_letters_pressed - 1
	get_tree().get_root().get_node("CaesarCipher").find_node(label_dict.get(num_letters_pressed, "Blank23")).set_text("_")
...
func _on_Next_pressed():
	get_tree().change_scene("res://Scenes/Caesar/CCDecryptTutorial.tscn")
